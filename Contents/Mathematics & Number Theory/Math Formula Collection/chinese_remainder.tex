...
pair<long long, long long> crt(long long a1, long long m1,
    ...
    long long m1_reduced = m1 / g;
    long long m2_reduced = m2 / g;

    long long diff = (a2 - a1) / g % m2_reduced;
    if (diff < 0) diff += m2_reduced;

    long long inv = mod_pow(m1_reduced, m2_reduced - 1, m2_reduced);
    long long step = diff * inv % m2_reduced;
    long long x0 = (a1 + step * m1) % lcm;
    if (x0 < 0) x0 += lcm;
    return {x0, lcm};
}
\end{lstlisting}
