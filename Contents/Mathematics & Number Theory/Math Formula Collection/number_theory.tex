...
\noindent
\textbf{Modular Inverse Basics}
對於 $\gcd(a,m)=1$，存在唯一 $a^{-1} \in [0,m)$ 使得 $a \cdot a^{-1} \equiv 1 \pmod m$。
\begin{itemize}
  \item \textit{用途：} 實作模除法、解線性同餘、組合數除法、CRT 等。
  \item \textit{求法：} 擴展歐幾里得適用任意模數；若 $m$ 為質數可用 $a^{m-2} \bmod m$。
  \item \textit{存在條件：} 僅當 $\gcd(a,m)=1$；不互質時可除以 $d=\gcd(a,m)$ 後再檢查。
\end{itemize}
\begin{lstlisting}
    g = gcd(a, m, c, x, y) ;
    if(g == 1){
...
\noindent
根據 貝祖定理，只有當 $d \mid c$ 時，$ax + by = c$才有整數解

\noindent
對於 $ax + by = c$ 假設有整數解，且 $gcd(a, b) = 1$
使用exgcd求出 $(a_0, b_0)$ 的特解後，根據逆模元可得出
通解: $a = a_0 + \frac{b}{d} t$, $b = b_0 - \frac{a}{d} t$
其中 t 可以為任意整數